...
export default function CatalogSort({ currentPath, catalogSortState, setCatalogSortState }: CatalogSortPropsType) {
  ...
  const onSortOrderListClickHandler = useCallback((evt: SyntheticEvent<HTMLElement>) => {
    ...
    const sortOrder = targetSortOrderElement.getAttribute('data-sortorder') as SortOrderType;
    setCatalogSortState({
      ...catalogSortState,
      sortOrder: sortOrder,
    });
  ...
  return (
    <div className="catalog-sort">
      ...
      <div ref={sortTypeListRef} className="catalog-sort__type" onClick={onSortTypeListClickHandler} >
        { currentPath === RoutePathEnum.ProductList && <button className="catalog-sort__type-button" data-sorttype={SortTypeEnum.SortDate} aria-label="по цене">по дате</button> }
        <button className="catalog-sort__type-button catalog-sort__type-button--active" data-sorttype={SortTypeEnum.SortPrice} aria-label="по цене">по цене</button>
        <button className="catalog-sort__type-button" data-sorttype={SortTypeEnum.SortRating} aria-label="по популярности">по популярности</button>
      </div>
      <div ref={sortOrderListRef} className="catalog-sort__order" onClick={onSortOrderListClickHandler} >
        <button className="catalog-sort__order-button catalog-sort__order-button--up catalog-sort__order-button--active" data-sortorder={SortOrderEnum.SortAsc} aria-label="По возрастанию" />
        <button className="catalog-sort__order-button catalog-sort__order-button--down" data-sortorder={SortOrderEnum.SortDesc} aria-label="По убыванию" />
      </div>
    </div>
  );
}
